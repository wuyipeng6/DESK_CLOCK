...
/**
  * @brief  硬件I2C写数据封装
  * @param  Reg 控制字节：0x00(命令) 或 0x40(数据)
  * @param  Data 数据数组首地址
  * @param  Count 要发送的字节数
  */
void OLED_I2C_Write(uint8_t Reg, uint8_t *Data, uint16_t Count)
{
    // 1. 等待总线空闲 (Busy位不适合用WaitEvent检查，因为它不是一个Event)
    uint32_t Timeout = 10000;
    while(I2C_GetFlagStatus(I2C1, I2C_FLAG_BUSY) && Timeout--);
    if(Timeout == 0) {
        printf("OLED Error: Bus Busy before start!\r\n");
        return;
    }

    // 2. 发送起始位
    I2C_GenerateSTART(I2C1, ENABLE);
    if(WaitEvent(I2C1, I2C_EVENT_MASTER_MODE_SELECT, "Start Bit (EV5)")) return;

    // 3. 发送从机地址
    I2C_Send7bitAddress(I2C1, 0x78, I2C_Direction_Transmitter);
    if(WaitEvent(I2C1, I2C_EVENT_MASTER_TRANSMITTER_MODE_SELECTED, "Slave Address (EV6)")) return;

    // 4. 发送控制字节 (命令或数据首地址)
    I2C_SendData(I2C1, Reg);
    // 使用 TRANSMITTING (TXE) 以准备后续数据的快速填充
    if(WaitEvent(I2C1, I2C_EVENT_MASTER_BYTE_TRANSMITTING, "Control Reg (EV8)")) return;
    ...
    for (uint16_t i = 0; i < Count; i++)
    {
        ...
        // 优化：使用 TRANSMITTING (EV8 / TXE) 而非 TRANSMITTED (EV8_2 / BTF)
        // TXE置位时，数据已搬移至移位寄存器，此时即可写入下一字节，提高吞吐量
        if(WaitEvent(I2C1, I2C_EVENT_MASTER_BYTE_TRANSMITTING, "Data Byte (EV8)")) return;
    }

    // 6. 在发送停止位前，确保最后一个字节彻底发送完成 (BTF)
    // 这是一个稳妥的做法，确保物理电平已完成传输后再释放总线
...
// 修改原本 Page 4 的 OLED_WriteCommand
void OLED_WriteCommand(uint8_t Command)
{
	OLED_I2C_Write(0x00, &Command, 1);
}

// 修改原本 Page 4 的 OLED_WriteData
